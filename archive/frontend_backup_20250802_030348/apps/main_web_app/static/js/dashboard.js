 ...
 * ========================
 * 
 * 메인 dashboard 화면의 모든 동적 기능 담당
 * - 모듈화된 dashboard 클래스 사용
 * - 시스템 data loading 및 표시
...
class LegacyDashboard {
    constructor() {
        this.summaryData = null;    // API에서 로드한 summary 데이터
        this.init();                // dashboard 초기화 시작
    }

    /**
     * dashboard 초기화 메인 함수
     * 모든 data loading 및 UI 업데이트를 순차적으로 실행
     */
    async init() {
        try {
            Utils.showLoading();                        // 로딩 스피너 표시
            await this.loadSummaryData();               // API에서 summary 데이터 로드
            await this.loadCharts();                    // 차트 데이터 로드 및 렌더링
            this.updateSummaryCards();                  // summary 카드 업데이트 (모델 수, 데이터셋 수 등)
